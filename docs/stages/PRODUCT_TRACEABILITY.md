...
Источник: `docs/product/PRODUCT_SPEC.md` version 3.30
...
| 5.6.5 | Write grants in shared DB (TZ 3.21 / **3.30**): `(user_id, card_path)` or `(user_id, tag)` or `(user_id, path_prefix)`; eval per request; **empty-list change:** empty = no extra shared write, not whole queue; same primitive for editor slice and user N-cards; **one server file NOW**; personal = ungranted drafts; vault = client; admin «Доступы» | `access_grants` + admin CRUD this wave; two-table copies leftover |
...
| Per-card display API + card rights | TZ **3.24** narrowed by **3.30**: endpoints per card; authorization is DB grant `(user, path)` / `(user, tag)` / `(user, prefix)`, not «rights live on the card». Grant filter / `access_grants` this wave. Open: verbs; paid maker vs user/editor; ADR-016 vs grant; amend vs supersede ADR-007; read-without-write for queue; grant by original author. Needs ADR with 3.26 one-plugin supersede of 3.09. |
