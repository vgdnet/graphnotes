...
Merge into the shared rhizome is governed by **Differ's merge rules**, not by
GitHub Pull Request UX and not by last-write-wins in PostgreSQL. The user
proposes a selected Differ subset; editors accept, reject, return or roll
back; self-approval is forbidden by author id; publication is atomic; MVP
covers adds and updates only. See `PRODUCT_SPEC` §6.6.1.
...
- `PRODUCT_SPEC` 1.8 names Differ as the entity that both compares personal →
  published shared and owns the merge-into-shared rules for selected results.
