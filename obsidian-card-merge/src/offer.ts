...
/** Explorer titles omit `.md`; TFile.extension is still `md`. */
export function offerMarkdownPath(
  ...
  const path = raw.toLowerCase().endsWith('.md') ? raw : ext === 'md' ? `${raw}.md` : '';
  if (!path) return null;
  return eligible(path) ? path : null;
}

export function isOfferMarkdownFile(file: { extension?: string; path?: string } | null | undefined): boolean {
  return offerMarkdownPath(file) != null;
}
