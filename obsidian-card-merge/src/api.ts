...
export class Api {
  ...
  async granted(): Promise<RemoteFile[]> {
    const data = obj(await this.json('/granted'));
    if (!Array.isArray(data.items)) throw new Error('GET granted не вернул items.');
    return data.items.map(parseFile);
  }

  async grantedContent(path: string): Promise<FileContent> {
    return parseFileContent(path, await this.exchange(`/granted/files/content?path=${encodeURIComponent(path)}`));
  }

  async putGranted(path: string, bytes: Uint8Array): Promise<RemoteFile> {
    const response = await this.exchange(
      `/granted/files?path=${encodeURIComponent(path)}`,
      'PUT',
      undefined,
      bytes,
    );
    if (!response.bytes.byteLength) throw new Error('PUT granted не вернул JSON.');
    try {
      return parseFile(JSON.parse(new TextDecoder().decode(response.bytes)));
    } catch {
      throw new Error('PUT granted вернул не JSON.');
    }
  }
}
