...
import { Api, canProposeToRhizome } from './api';
import {
  OFFER_BUTTON_LABEL,
...
export interface SidebarHost {
  saved: { server: string; token: string };
  canPropose: boolean | undefined;
  proposeDenied: boolean;
  connect(signal: AbortSignal): { api: Api };
...
export class GraphNotesSyncView extends ItemView {
  ...
  async onOpen(): Promise<void> {
    ...
    root.createEl('p', {
      cls: 'gn-muted',
      text: 'Локальные правки уходят в личное хранилище. Общая ризома не меняется с этой кнопки.',
    });
  ...
  private async mountOfferPanel(): Promise<void> {
    const host = this.offerHost;
    if (!host) return;
    let allowed = false;
    if (this.plugin.saved.server.trim() && this.plugin.saved.token.trim()) {
      try {
        const { api } = this.plugin.connect(this.plugin.beginWork());
        const caps = await api.capabilities();
        const role = typeof caps.user.role === 'string' ? caps.user.role : '';
        allowed = canProposeToRhizome(
          role,
          typeof caps.can_propose_to_rhizome === 'boolean' ? caps.can_propose_to_rhizome : undefined,
        );
        this.plugin.canPropose = allowed;
        this.plugin.proposeDenied = !allowed;
      } catch {
        allowed = this.plugin.canPropose === true;
      }
    }
    if (!allowed) {
      host.empty();
    ...
    const refresh = actions.createEl('button', { text: 'Обновить список' });
    ...
    this.statusEl = host.createEl('p', { cls: 'gn-muted' });
  ...
  private setStatus(text: string, error = false): void {
    ...
    if (error) this.statusEl.addClass('gn-error');
    else this.statusEl.removeClass('gn-error');
  }
  ...
  private renderList(): void {
    ...
    if (!this.offers.length) {
      this.listEl.createEl('p', { cls: 'gn-muted', text: 'Нет карточек, которых нет в общей или которые отличаются.' });
      return;
    }
    for (const offer of this.offers) {
      ...
      if (this.queued.has(offer.path)) {
        row.createEl('span', { cls: 'gn-muted', text: 'Уже в очереди' });
        continue;
      }
...
function describeError(error: unknown): string {
  return describeOfferError(error);
}
