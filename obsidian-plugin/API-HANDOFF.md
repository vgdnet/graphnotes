...
Плагин копит правки локально и пишет в личное по кнопке / закрытию файла / минутам (ТЗ 2.90), не на каждый `modify`. Общую ризому не трогает. ТЗ 2.88: Differ в API отдаёт отличия и просьбы «карточки нет в общей». Сначала кабинет; в плагин — следующая итерация, тем же `GET /api/differ`, не вторая сверка. `docs/OBSIDIAN_PLUGIN_API_TZ.md` — не канон: PLG-02/03 («сохранение не шлёт», обязательный выбор) сняты.
